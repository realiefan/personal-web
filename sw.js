...
const CACHE_VERSION = "V1.3"; // Update the cache version when you make changes to the caching logic
    ...
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter(
            (cache) =>
              cache.startsWith(CACHE_PREFIX) && !cache.includes(CACHE_VERSION)
          )
